...
extension UInt8 {
  func isValidEscPosStatusByte() -> Bool {
    (self & Self.fixedBitsMask) == Self.fixedBitsPattern
  }

  /// Bits 0, 1, 4 and 7 — the positions ESC/POS fixes on every status byte.
  private static let fixedBitsMask: UInt8 = 0x93

  /// bit0=0, bit1=1, bit4=1, bit7=0 — the only value those positions may take.
  private static let fixedBitsPattern: UInt8 = 0x12
}
