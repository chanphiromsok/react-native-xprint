 ...
 * when I call print()" three files away from the actual mistake. Most
 * screens will not call this directly — `usePrinter` is the everyday hook —
 * but it is exported for the rare screen that needs the session itself,
 * e.g. `PrinterSettingsScreen` calling `session.select`/`session.forget`
 * directly.
 */
